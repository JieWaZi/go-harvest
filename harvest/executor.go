...
type Executor struct {
	...
	PreFunction JobFunc
	// 各执行函数
	FunctionMap map[string]JobFunc
	// 初始Url
	InitialUrl string
	// 初始函数
...
type JobFunc func(ctx *ExecutorContext) error

// TaskConfig is the config of a executor
...
// Option is the config option of a executor
type Option struct {
	UserAgent              string
	MaxDepth               int
...
// Limit is the limit of a executor
type Limit struct {
	Enable bool
...
func newCollector(config ExecutorConfig) (*colly.Collector, error) {
...
func (e *Executor) execFunction(functionName string, context *ExecutorContext) error {
	if functionName == "" {
		return nil
	}
	if _, ok := e.FunctionMap[functionName]; !ok {
		return ErrFunctionNameNotFind
	}

	return e.FunctionMap[functionName](context)
}
